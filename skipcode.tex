...
\node[linecode] at ($(locate-head.west)+(9pt,-20pt)$)  {\scriptsize bool marked = false;};
...
\node[linecode] at ($(locate-head.west)+(36pt,-90pt)$)  {\scriptsize    curr = pred.next[i]; };
\node[linecode] at ($(locate-head.west)+(36pt,-100pt)$)  {\scriptsize    while (true) };
\node[linecode] at ($(locate-head.west)+(45pt,-110pt)$)  {\scriptsize      succ = curr.next[i].get(marked); };
 \node[linecode] at ($(locate-head.west)+(45pt,-120pt)$)  {\scriptsize     while (marked) };
 \node(node4)[linecode] at ($(locate-head.west)+(54pt,-130pt)$)  {\scriptsize       snip = CAS(pred.next[i], curr, succ, false, false); };
\node[linecode] at ($(locate-head.west)+(54pt,-140pt)$)  {\scriptsize        if (!snip) continue retry; };
...
\end{tikzpicture}
\caption{A $\tt find$ method of a lock-free skiplist-based concurrent set.}

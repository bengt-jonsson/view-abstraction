...
%%%%%%%%%%% struct %%%%%%%%%%%%%%%%
\node[linecode,anchor=south west](struct) at ($(top-head.east)+(-30pt,0pt)$) { \scriptsize \textbf{struct} Node \scriptsize \{};
%{struct Node(bool lock; int val; Node *next; bool mark);};
\node(struct1)[linecode] at ($(top-head.west)+(10pt,-5pt)$) {\scriptsize \textbf{int} data;};
...
\node(struct4)[linecode] at ($(top-head.west)+(10pt,-37pt)$) {\scriptsize \textbf{boolean} mark;};
...
\node(locate-head)[linecode] at ($(top-head.east)+(143pt,8pt)$) {\scriptsize \textbf{int} pop():};

\node(locate1)[linenum,anchor=west] at ($(locate-head.west)+(0pt,-10pt)$) {\scriptsize 1};
\node[linecode] at ($(locate1.east)+(0pt,0pt)$) {\scriptsize \textbf{boolean} success = false;};

\node(locate2)[linenum] at ($(locate1.east)+(0pt,-10pt)$) {\scriptsize 2};
\node[linecode] at ($(locate2.east)+(0pt,0pt)$) {\scriptsize \textbf{int} maxTS = -1;};
...
\node(code4)[linecode] at ($(locate4.east)+(0pt,0pt)$) {\scriptsize \textbf{while} (!success)}; 

\node(locate5)[linenum] at ($(locate4.east)+(0pt,-10pt)$) {\scriptsize 5};
\node[linecode] at ($(locate5.east)+(7pt,0pt)$) {\scriptsize \textbf{int} k; };

\node(locate6)[linenum] at ($(locate5.east)+(0pt,-10pt)$) {\scriptsize 6};
\node[linecode] at ($(locate6.east)+(7pt,0pt)$) {\scriptsize \textbf{for}(\textbf{int} i=1; i<=maxThreads; i++)};

\node(locate7)[linenum] at ($(locate6.east)+(0pt,-10pt)$) {\scriptsize 7};
\node[linecode] at ($(locate7.east)+(14pt,0pt)$) {\scriptsize n = pools[i];};

\node(locate8)[linenum] at ($(locate7.east)+(0pt,-10pt)$) {\scriptsize 8};
\node(code8)[linecode] at ($(locate8.east)+(14pt,0pt)$) {\scriptsize \textbf{while} (n.mark $\&$ n.next != n) n = n.next; };

\node(locate9)[linenum] at ($(locate8.east)+(0pt,-10pt)$) {\scriptsize 9};
\node[linecode] at ($(locate9.east)+(14pt,0pt)$) {\scriptsize \textbf{if}(maxTS < n.ts)};
...
\node[linecode] at ($(locate12.east)+(21pt,0pt)$) {\scriptsize k = i; myTop = pools[k];};

\node(locate13)[linenum] at ($(locate12.east)+(0pt,-10pt)$) {\scriptsize 13};
\node[linecode] at ($(locate13.east)+(7pt,0pt)$) {\scriptsize \textbf{if} (youngest != null)};
...
\node[linecode] at ($(locate15.east)+(21pt,0pt)$) {\scriptsize \textbf{if} (success)};
...
\node[linecode] at ($(locate17.east)+(28pt,0pt)$) {\scriptsize \textbf{if} (myTop != youngest);};
...
\node(code22)[linecode] at ($(locate22.east)+(28pt,0pt)$) {\scriptsize \textbf{while} (next.next != next $\&$ next.mark);};
...
\node[linecode] at ($(locate25.east)+(0pt,0pt)$) {\scriptsize \textbf{return} youngest.data;};
...
{\scriptsize \textbf{while} (next.next != next $\&$ !next.mark)};
...
\node(init2)[linecode] at ($(top-head.east)+(-30pt,-90pt)$) {\scriptsize \textbf{for}(\textbf{int} i=1; i<=maxThreads; i++)};
...
\caption{Description of the Timestamped stack algorithm, with some simplifications.}
\label{ts-stack:fig}
\end{figure}

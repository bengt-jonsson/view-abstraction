...
It can be shown \bjcom{IN WHICH CITATION?} that this is equivalent to
...
\todo[inline]{This subsection needs compactification and revision}
For preciseness, we outline how we formalize the semantics of programs and
observers. We assume a program $\prog$
...
variables $\glvarset$ to their values, and
(iv)
...
$\lstate$ and a heap $\heap$, with transitions of the form:
...
$\lbl$ is empty, except when the executed statement is annotated as
...
transition $\tuple{\ostate,\returntwo{\mname}{l},\ostate'}$.
...
\todo[inline]{Here, we need to state a the theorem that we have reduced linearizability checking to reachability}

...
Thereafter, an {\em observer} is constructed, which
...
~\cite{BEEH:icalp15,CHSV:aspect-oriented} that the set of linearizable histories,
...
%% ~\cite{BEEH:icalp15,CHSV:aspect-oriented} that the linearizability criterion can be
...
%% \end{quote}
%% The complement of each such constraint can be
%% expressed by an {\em observer}, as introduced in~\cite{AHHR:integrated:short}. 
%% %
%% Observers are
%% finite automata extended with a finite set of {\em registers}
...
%% %
%Other
%observers for stacks and queues can be found in~\cite{AHHR:integrated:short}

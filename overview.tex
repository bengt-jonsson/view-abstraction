...
Now, let us describe an important method $\tt add$ of the algorithm. The method is to add a new node into the list.  Figure~\ref{sl-code:fig} shows the codes of the method.
...
In the heap state of Figure~\ref{sl-shape}, thread $\thread_1$ is trying to add a new node with key $9$. \bjcom{It may be better to say at which line in the code
  they are executing}It has done the first iteration of the for loop in it's $\tt find$ function. Thread $\thread_2$ is also doing the same thing as $\thread_1$ and it has done the second iteration. Thread $\thread_2$ is trying remove a node with key $9$ and it has done the third iteration.
\bjcom{But you do not show the code of remove?}

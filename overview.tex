...
lowest-level list, and serve as shortcuts into lower-level lists. Figure ~\ref{normalskiplist} shows an example of a skiplist of height 3. In the figure, head and tail are two nodes of height $3$, there are two nodes of height $2$ and one node of height $1$
...
In the algorithm, each heap cell has a {\tt key} field, a {\tt height} field, an array of {\tt next} pointers, indexed from
$1$ up to its {\tt height}, and a {\tt marked} field which is true if the node has been logically removed. The removal of a node occurs in two steps, first the node is logically removed by setting its marked flag to {\tt true}, then the node is physically removed by unlinking it from the list. The algorithm must be able to update $\tt next[i]$ and $\tt marked[i]$ atomically, this standard technique is implemented by encoding them in a single word. The head and tail cells of the skiplist are pointed to by global pointer variables $\tt H$ and $\tt T$, respectively. The $\tt find$ method traverses the list at decreasing levels using two local variables $\tt pred$ and $\tt curr$, starting at the head and at 
the maximum level (line 5). At each level $\tt i$ it sets $\tt curr$ to $\tt pred.next[i]$ (line 7). During the traversal, a pointer variable $\tt succ$ and data variable $\tt marked$ are atomically set to be $\tt curr.next[i]$ and $\tt curr.marked[i]$ respectively (line 9, 14). After that, the method repeatedly removes marked nodes at the current level (lines 10 to 14). This is done by using a $\tt compareAndSwap (CAS)$ command (line 11). The $\tt CAS$ command tests whether $\tt pred.next[i]$ and $\tt pred.marked[i]$ are equal to $\tt curr$ and {\tt false} respectively, and if both tests succeed, it replaces them with $\tt succ$ and {\tt false} and returns {\tt true}; otherwise, the $\tt CAS$ returns {\tt false}. During the traversal, $\tt pred$ and $\tt curr$ are advanced until $\tt pred$ points to a node with the largest key on that level smaller than $\tt x$ (line 18). Then, obtained values of $\tt pred$ and $\tt curr$ are recorded into $\tt preds[]$ and $\tt succs[]$ at the current level (lines 19, 20), and thereafter traversal continues one level below until it reaches the bottom level. Finally, it returns {\tt true} if the {\tt key} value of $\tt curr$ is equal to {\tt x}, otherwise, it returns {\tt false} meaning that the node with key $\tt x$ is not found.
...
The $\tt add$ method uses $\tt find$ to check whether a node with key $\tt x$ is already in the list. Id so it returns $\tt false$; otherwise, a new node is created with randomly chosen height $\tt h$ (line 7) and its next pointers at levels from $1$ to $\tt h$ are initialised to $\tt succ$ (line 8 to 9). Thereafter, the new node is added into the list. This is performed by linking it into the bottom-level list between $\tt preds[1]$ and $\tt succs[1]$ nodes returned by $\tt find$. This is performed by using a $\tt CAS$ to set the next reference while validating that these nodes still refer one to the other and have not been removed from the list (line 13). If $\tt CAS$ fails, the method will restart from line 3 by calling $\tt find$ again. Otherwise, the node is added into the list. Thereafter, the $\tt add$ proceeds with linking the new node into increasingly higher levels. (lines 16 to 22). For each higher level, the method $\tt find$ (line 22) is called again to recompute the $\tt preds[]$ and $\tt succs[]$ on the remaining unlinked levels. Once all levels are linked, the method returns {\tt true} (line 23).
...
Our abstraction includes two kinds of fragments representing the heap in Figure ~\ref{sl-shape} in the view of $\thread_1$, one is for level 1 and the other is for higher levels. For example, the first pair of heap cells with keys $\tt -\infty$ and $5$ in the bottom-level list is represented by $\frag_1$ where the variable $\tt preds(1)[3]$ is represented by $\tt preds[higher]$. The pair of heap cells with keys $\tt -\infty$ and $\tt +\infty$ in the level 3 is represented by $\frag_9$. In each fragment, $\vals$ is represented inside each tag node of the fragment. The $\datarelat{\frag}$ is above the arrow between two tags. The first row under each tag is $\reachfrom$, whereas the second row is $\reachto$. Above each tag node is $\pvars$.
...
these ($\frag_6$, $\frag_7$ and $\frag_{11}$) consist of a tag that points to $\dangconst$. Whereas, the other fragments consist of a pair of pointer-connected tags. The fragments $\frag_1$, \ldots , $\frag_{6}$ are abstracted from the level $1$, they represent pairs of heap cells that are connected by a pointer field appearing at level 1. Whereas, the other fragments are abstracted from higher levels. The private value of the input tag of $\frag_7$ is {\tt true}. Whereas, the private values of other tags of other fragments are {\tt false}. A verification of linearizability of the algorithm must represent several key

...
Here is the list of several properties of the algorithm: 
\begin{itemize}
	\item It never traverses a marked link. Instead, it removes the node referred to by a
marked link from the list at that level.
\item Every preds[] reference is to a node with a key strictly less than the target.
\item If the node is physically removed before the $\tt find$ starts, then it will not be found

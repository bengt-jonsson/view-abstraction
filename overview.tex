...
A skiplist has sentinel head and tail nodes with maximum heights key values $-\infty$ and $+\infty$, respectively. %Figure~\ref{sl} shows a skiplist with maximum height of 3.
%\begin{figure}
%\center  
% \input skiplist
...
There are three main methods of the algorithm namely $\tt add$, $\tt contain$ and $\tt remove$. All methods are relied on a method called $\tt find$. In this section, we describe shortly about $\tt find$ and $\tt add$ of the algorithm. Figure~\ref{sl-code:fig} shows the codes of the method.
...
Each node of the list has a key and a $\tt marked$ field which indicate whether it is in the list, or has been logically removed. 
The $\tt find$ method traverses the skipList using two pointer variables $\tt pred$ and $\tt curr$ starting from the head and at the highest level. It then proceeds in each level down the list, filling in $\tt preds$ and $\tt succs$ nodes that are repeatedly advanced until $\tt pred$ points to a node with
the largest value on that level that is smaller than the $\tt x$. The method repeatedly remove marked nodes from the
given level(Lines 15 to 19) by using a $\tt compareAndSet$ statement. The $\tt compareAndSet$ statement has two arguments: an expected value and an update value. If the current register value is equal to the expected value, then it is replaced by the update value; otherwise the value is unchanged. The method call returns a Boolean indicating whether the value changed. Once an unmarked $\tt curr$ is found (Line 20), it is tested to see if its key is less than the target key. If so, $\tt pred$ is advanced to $\tt curr$. Otherwise, $\tt curr$'s key is greater than or equal to $\tt x$, so the current value
of $\tt pred$ is the target node's immediate predecessor. The method breaks out of the current level search loop, saving the current values of $\tt pred$ and $\tt curr$. It proceeds by this way until reaching the bottom level. The $\tt add$ method for adding a node with the key $\tt k$ works as follows:
...
The $\tt find$ method traverses the SkipList using two pointer variables $\tt pred$ and $\tt curr$ which start from the head and at the highest level. It then proceeds in each level down the list, filling in $\tt preds$ and $\tt succs$ nodes that are repeatedly advanced until $\tt pred$ points to a node with
...
of $\tt pred$ is the target node's immediate predecessor. The find method breaks out of the current level search loop, saving the current values of $\tt pred$ and $\tt curr$. It proceeds by this way until reaching the bottom level. The $\tt add$ method for adding a node with the key $\tt k$ works as follows: 
...
\bjcom{Put these LPs as dots in the code. Are these really the LPs?}
...
\todo[inline]{Maybe we should omit observers from this section?}
{\em Observers} are
...
In the heap state of Figure~\ref{sl-shape}, thread $\thread_1$ is trying to add a new node with key $9$ and it is in the line $8$ of the code of $\tt add$.  Thread $\thread_2$ is also doing the same thing as $\thread_1$ and it has done its first iteration of the loop in $\tt find$ method in line $25$. One of the main properties of the algorithm is that 
...
Figure~\ref{fig:skiplistabs} shows a set of fragments that is

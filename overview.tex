...
the maximum level. At each level it sets $\tt curr$ to be the $\tt pred$ node’s successor (line 7). After that, the method repeatedly removes marked nodes at the current level (lines 8 to 18). This is done as following: A pointer variable $\tt succ$ and data variable $\tt marked$ are atomically set to be successor and marked field value of $\tt curr$ respectively (line 9, 14). Next, while the value of $\tt marked$ is {\tt true}, the marked $\tt curr$ is removed by using a $\tt compareAndSwap(CAS)$ command (line 11). The $\tt CAS$ command tests the expected reference and mark values of $\tt pred$ are equal to $\tt curr$ and {\tt false} respectively, and if both tests succeed, replaces them with $\tt succ$ and {\tt false} and return {\tt true}. Otherwise,  the $\tt CAS$ return {\tt false}. These atomic assignment and $\tt CAS$ command are standard technique in concurrent program where they encode both addresses in a single word.  During the traversal, the two pointer variables $\tt pred$ and $\tt curr$ are advanced until $\tt pred$ points to a node with the largest key on that level smaller than $\tt x$ (line 19), recording the obtained values of  $\tt pred$ and $\tt curr$ into the arrays $\tt preds$ and $\tt succs$ at the current level (line 20,21), and thereafter continuing one level below.

...
(lines 15 to 19) using a $\tt compareAndSet$ command. The $\tt compareAndSet$ command tests the expected reference and mark values, and if both tests succeed, replaces them with updated reference and mark values.  
...
successor $\tt succ$ (line 10). The the next step is to add the new node into the list. It is performed by linking it into the bottom-level list between the $\tt preds[1]$ and $\tt succs[1]$ nodes returned by $\tt find$ by using a $\tt compareAndSet$ to set the reference while validating that these nodes still refer one to the other and have not been removed from the list (line 14). If $\tt compareAndSet$ fails, the method call will restart. Otherwise, the node is added into the list. Thereafter, the $\tt add$ proceeds with linking the new node into increasingly higher levels.
...
In Figure~\ref{sl-code:fig}, the LP of a successful {\tt add} operation is at line 16 of the $\tt add$ method (denoted by a blue dot), whereas the linearization point of an unsuccessful {\tt add} operation is at line 7 of the $\tt add$
...
\begin{itemize}
\item Bottom-list is sorted. Two unmarked nodes cannot have the same value.
\item Higher-level lists are shortcuts
\item unreachable nodes are marked
\item the variables pred, curr, preds, currs reflect the position of the
  cell with {\tt key}
\end{itemize}

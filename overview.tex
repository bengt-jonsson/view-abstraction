...
that appear before (line 15-16) or after (line 18-21) the removed cell.
...
Thread $\thread_2$ has reached line XXX, in the execution of the {\tt pop} method,  and has just assigned {\tt youngest} to the first node in the list
...
\todo[inline]{Comments on Figure~\ref{fig:tsshape}: Quy, could you make some small
  modifications:
(1)
    In the array {\tt pools}: Could you remove the indices $\tt \thread_i$
    inside the array fields, and just put the number ($i$) to the left of each
    field.
(2) The variables $\tt top_i$ do not exist. I suggest to remove them,
    and replace by $\tt pools[i]$ (see my new text).
}
...
\todo[inline]{to Quy: Variable {\tt n} is never declared. Can you fix?}
...
satisfies the local symbolic configuration which maps
\todo[inline]{to Quy: Can you add the values of observer registers, and
...
Let us now show how our fragment abstraction represents the view of thread
$\thread_2$.
...
the non-pointer fields of $\cell$ satisfies the abstraction $\vals$, i.e.
  ...
  \item
    ...
    for each observer register $\reg_i$ a, if
    the value $ts'$ of the {\tt ts} field of some $\reg_i$-cell
    satisfies $ts \sim ts'$, then $\sim \in \vals({\tt ts})$,
  \end{inparaenum}

...
the verification requires the user to annotate how linearization
points are placed in each method. For the skiplist algorithms, this
is done by affixing   linearization points to particular statements.
...
In more complex cases, we use
  ...
  For concurrent implementations of stacks and queues, no user instrumentation
  is necessary, winc   linearizability can be precisely specified by
  observers that synchronize on call and return actions of
  methods, as shown by~\cite{BEEH:icalp15,HSV:concur13}.
...
In the case of set, we use observers to the constraint that intuitively
expresses that
%\todo[inline]{Quy: Make sure that you have the right constraints. I just guessed that the below four are the right ones}
for any data item, successful insertions and removals must alternate,
  starting with insertion.
It is straight-forward to construct an observer
...
To verify that the skiplist algorithm is a correct linearizable implementation
of a set, we
...
constraints. To verify a constraint, 
...
observer, where the observer synchronizes with the program on the designated
linearization points. This reduces the
...
a shape abstraction, and data abstraction.
\begin{itemize}
  \item
  Our thread abstraction employs the thread-modular approach by representing only the view of single, but arbitrary, thread $\thread$. Such a view consists of
the local state of thread $\thread$ and the part of the heap that is accessible to thread $\thread$ via its local pointer variables or via global pointer variables.
\item
The accessible part of the heap is represented by  
a set of {\em fragments}. Each fragment is an abstraction of a pair of
nodes that are connected by a pointer. The abstraction represents
\begin{itemize}
\item the contents of the data fields in the pair of nodes,
\item which pointer variables (either global or local to $\thread$)
  point  to the nodes,
\item and also represents global information, which expresses how
  ...
  global pointer variable or if it triggers some noteworthy behavior in
  a thread that reaches it: for our example this happens for nodes whose
  {\tt key} field has the  same value as the observer register.
\end{itemize}
\item
Finally, a natural data abstraction is applied to the non-pointer local
variables of the thread and the non-pointer fields of each fragment. For our
...
data values are represented by constraints over their relative ordering
(wrp.\ to the order $<$),
as well as their ordering with respect to the value of the observer register.
\end{itemize}
\todo[inline]{Here should be a motivation why this abstraction is good,
in particular focussing on the fragment abstraction}

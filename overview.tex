...
node[right=2pt,pos=0.5] {\footnotesize${\tt act}$} 
(s2);
...
\end{tikzpicture}
\caption{An observer recognizing differentiated traces which have a Pop with no corresponding Push. The variable $\tt x$ is an observer register, The action $\tt act$ ranges over all actions associated with $\tt x$ }
...
\draw[->,>=stealth,line width=0.5pt] (s0) to 
node[above = 0pt,pos=0.5, sloped] {\tiny${\tt act_1}$} 
(s7);
\draw[->,>=stealth,line width=0.5pt] (s1) to 
node[above=0pt,pos=0.5,  sloped] {\tiny${\tt act_2}$} 
(s7);
\draw[->,>=stealth,line width=0.5pt] (s2) to 
node[above=0pt,pos=0.5,  sloped] {\tiny${\tt act_3}$} 
(s7);
\draw[->,>=stealth,line width=0.5pt] (s3) to 
node[above=0pt,pos=0.5,  sloped] {\tiny${\tt act_4}$}  
(s7);
\draw[->,>=stealth,line width=0.5pt] (s4) to 
node[above=0pt,pos=0.5,  sloped] {\tiny${\tt act_5}$}  
(s7);
\draw[->,>=stealth,line width=0.5pt] (s5) to 
node[above=0pt,pos=0.5,  sloped] {\tiny${\tt act_6}$}  
(s7);
...
The thread $\tt T_1$ is trying to push a new node pointed by its local variable $\tt new[1]$ while $\tt T_3$ has just called its push method and $\tt T_2$ is doing its pop method. The thread $\tt T_2$ find the youngest node in the list owned by $\tt T_3$ which is determined by its local variable $\tt youngest[2]$.
...
\item
  ...
  {\tt $\tt top_3$, youngest[2], n[2], myTop[2]}

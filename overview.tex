...
(lines 15 to 19) using a $\tt compareAndSet$ command. A  $\tt compareAndSet$ command has two arguments: \bjcom{This needs fixing: I see 5 arguments} an expected value and an update value. If the current register value is equal to the expected value, then it is replaced by the update value; otherwise the value is unchanged. 

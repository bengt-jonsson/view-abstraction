...
lowest-level list, and serve as shortcuts into lower-level lists. Figure ~\ref{normalskiplist} shows an example of a skiplist of height 3. In the figure, There are two nodes of height $3$, two nodes of height $2$ and one node of height $1$
...
There are three main methods of the algorithm namely $\tt add$, $\tt contains$ and $\tt remove$. The method $\tt add(x)$ adds $\tt x$ to the set and returns true iff $\tt x$ was not already in the set; $\tt remove(x)$ removes $\tt x$ from the set and returns true iff ${\tt x}$ was in the set; and $\tt contains(x)$
...
In the algorithm, each heap cell has a {\tt key} field, {\tt height} field, an array of {\tt next} pointers, indexed from
$1$ up to its {\tt height}, and a {\tt marked} field which is true if the node has been logically removed. Logically removing is the fist step of removing a node which is done before the node is physically removed by unlinking it from the list. To ensure the correctness of the algorithm, atomic assignments where both next pointer and marked value are encoded in a single word are used in the algorithm, this is a standard technique used in concurrent programs. The head and tail cells of the skiplist are pointed by global pointer variables $\tt H$ and $\tt T$ respectively. The $\tt find$ method traverses the list at decreasing levels, starting at the head and at 
the maximum level (line 5). At each level $\tt i$ it sets $\tt curr$ to be the $\tt pred.next[i]$ (line 7). After that, the method repeatedly removes marked nodes at the current level (lines 10 to 14). This is done as following: A pointer variable $\tt succ$ and data variable $\tt marked$ are atomically set to be $\tt curr.next[i]$ and $\tt curr.marked[i]$ respectively (line 9, 14). Thereafter, the marked node pointed by $\tt curr$ is removed by using a $\tt compareAndSwap(CAS)$ command (line 11). The $\tt CAS$ command tests $\tt pred.next[i]$ and $\tt pred.marked[i]$ are equal to $\tt curr$ and {\tt false} respectively, and if both tests succeed, it replaces them with $\tt succ$ and {\tt false} and return {\tt true}. Otherwise, the $\tt CAS$ returns {\tt false}. During the traversal, $\tt pred$ and $\tt curr$ are advanced until $\tt pred$ points to a node with the largest key on that level smaller than $\tt x$ (line 18). Then,  obtained values of $\tt pred$ and $\tt curr$ are recorded into the arrays $\tt preds$ and $\tt succs$ at the current level (lines 19, 20), and thereafter continuing one level below. The $\tt find$ method proceeds this way until it reaches the bottom level. Finally, it will return {\tt true} if the {\tt key} value of $\tt curr$ is equal to {\tt x}, otherwise, it returns {\tt false}.
...
The $\tt add$ method uses $\tt find$ to check whether a node with key $\tt x$ is already in the list. It returns $\tt false$ if an unmarked node with key $\tt x$ is found. Otherwise, a new node is created with randomly chosen level $\tt h$ (line 7) and its next references at levels from $1$ to $\tt h$ are directed to the $\tt succ$ (line 8 to 10). Thereafter, the new node is added into the list. This is performed by linking it into the bottom-level list between the $\tt preds[1]$ and $\tt succs[1]$ nodes returned by $\tt find$. This is performed by using a $\tt CAS$ to set the next reference while validating that these nodes still refer one to the other and have not been removed from the list (line 14). If $\tt CAS$ fails, the method will $\tt find$ again (line 3). Otherwise, the node is added into the list. Thereafter, the $\tt add$ proceeds with linking the new node into increasingly higher levels. Thereafter, the $\tt add$ proceeds with linking the new node into increasingly higher levels (lines 17 to 23). The method $\tt find$ (line 23) is called again to recompute the ostensible $\tt preds$ and $\tt succs$ on the remaining unlinked levels. Once all levels are linked, the method returns {\tt true}.
...
\end{itemize}
Formally, A fragment $\frag$ is a triple of form
$\fragtupleat{\frag}$, of form
$\tuple{\inat{\frag},\nullconst}$, or of form
$\tuple{\inat{\frag},\dangconst}$,
where $\inat{\frag}$ and $\outat{\frag}$ are tags and $\datarelat{\frag}$ is a subset of $\set{<, =, >}$ that describes ordering information between two cells. A {\em tag} is a tuple $\atag = \tagtuple$, where
\begin{itemize}
\item
...
skiplist algorithm with three levels. Each heap cell is shown with the values of its fields as descried in the Figure~\ref{skipcell}. %\bjcom{Add a legend, showing the layout of fields, as in previous papers}
...
variable ${\tt preds[i]}$ of thread $\thread_j$, and its same for other local variables.
In the heap state of Figure~\ref{sl-shape}, thread $\thread_1$ is trying to add a new node with key $9$, and has reached line $8$ of the $\tt add$ method.  Thread $\thread_2$ is also doing the same thing as $\thread_1$ and it has done its first iteration of the loop in $\tt find$ method in line $19$.
...
We define two sets of fragments representing the heap in Figure ~\ref{sl-shape}, the first set is for level 1 and the other set is for higher levels. In our shape abstraction, we represent each fragment by a pair of
{\em tags}. 
...
%  \item $\private$ is a boolean value.
%\end{itemize}
%A fragment $\frag$ is a triple of form
%$\fragtupleat{\frag}$, of form
%$\tuple{\inat{\frag},\nullconst}$, or of form
%$\tuple{\inat{\frag},\dangconst}$,
%where $\inat{\frag}$ and $\outat{\frag}$ are tags and $\datarelat{\frag}$ is a subset of $\set{<, =, >}$ that describes ordering information between two cells.
We assume that
the value of the observer register is $9$. For example, the first pair of heap cells with keys $\tt -\infty$ and $5$ in the bottom-level list is represented by $\frag_1$ where the variable $\tt preds(1)[3]$ is represented by $\tt preds[higher]$. In the fragment, we have 
\begin{itemize}
...
\end{figure} 
Figure~\ref{fig:skiplistabs} shows fragments that represents the
part of the heap that is accessible to
$\thread_1$ in the configuration in Figure~\ref{sl-shape}. There are $10$ fragments, named $\frag_1$, \ldots , $\frag_{10}$. Two of
these ($\frag_5$ and $\frag_9$) consist of a tag that points to $\dangconst$,
and the other consist of a pair of pointer-connected tags. The fragments $\frag_1$, \ldots , $\frag_{6}$ are abstracted from the level $1$, they represent pairs of heap cells that are connected by a pointer field appearing at level 1. Whereas, the other fragments are abstracted from higher levels. In each fragment in Figure~\ref{fig:skiplistabs}, $\vals$ is represented inside each tag of the fragment. The $\datarelat{\frag}$ is above the arrow between two tags. The first row under each tag is $\reachfrom$, whereas the second row is $\reachto$. Above each tag is the $\pvars$.
...
As can be seen, all fragments are strictly sorted, therefore they present the first invariant. The second invariant is expressed by the fact that, all fragments in higher level are reachable from the global variable $\tt H$. The frag $\frag_{3}$ expresses the third invariant because it presents a marked node which is not reachable from the variable $\tt H$. Finally, the last invariant is expressed by the fact that $\tt x$ is larger than data fields of all tags containing $\tt pred$,$\tt preds$, and $\tt x$ is smaller than or equal to data fields of all tags containing $\tt curr$,$\tt currs$.     

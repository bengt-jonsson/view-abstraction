...
\textsf{Optimistic skip-list ~\cite{MS:QueueAlgorithms}}& \textsf{500} & \textsf{Michael lock-free set ~\cite{Michael:list}}  & \textsf{178} \\
\hline
 
\textsf{Priority queue skip-list 1 ~\cite{Shavit:ElimQueue}}  &  \textsf{1320} & \textsf{Pessimistic set  ~\cite{ArtOfMpP}}&\textsf{30}\\
\hline

\textsf{Priority queue skip-list 2~\cite{Shavit:ElimQueue}}  &  \textsf{599} &\textsf{Optimistic set ~\cite{ArtOfMpP}}& \textsf{25}\\
\hline
...
it for verifying concurrent algorithms (both lock-based and lock-free)

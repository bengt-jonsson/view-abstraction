...
\node[linecode,anchor=south west](struct) at ($(locate-head.north west)+(-4pt,8pt)$) 
{\scriptsize struct Node(int key; int topLayer; Node *nexts; bool marked; fullylinked;Lock lock);};
...
%%%%%%%%%% locate %%%%%%%%%%
\node(locate){};
\node(locate-head)[linecode] at ($(locate.west)+(-100pt,-10pt)$) {\scriptsize boolean find(int x, Node* preds[], Node* succs[]):};
\node[linecode] at ($(locate-head.west)+(0pt,-20pt)$)  {\scriptsize int mLevel = 0;};
\node[linecode] at ($(locate-head.west)+(0pt,-30pt)$)  {\scriptsize boolean marked[MAXLEVEL] = {false};};
\node[linecode] at ($(locate-head.west)+(0pt,-40pt)$)  {\scriptsize boolean snip;};
\node[linecode] at ($(locate-head.west)+(0pt,-50pt)$)  {\scriptsize Node* pred = null, curr = null, succ = null;};
\node[linecode] at ($(locate-head.west)+(0pt,-60pt)$)  {\scriptsize retry:};
\node[linecode] at ($(locate-head.west)+(7pt,-70pt)$)  {\scriptsize  while (true) };
\node[linecode] at ($(locate-head.west)+(14pt,-80pt)$)  {\scriptsize   pred = head; };
\node[linecode] at ($(locate-head.west)+(14pt,-90pt)$)  {\scriptsize   for (int i = MAXLEVEL; i >= mLevel; i--) };
\node[linecode] at ($(locate-head.west)+(21pt,-100pt)$)  {\scriptsize    curr = pred.next[i]; };
\node[linecode] at ($(locate-head.west)+(21pt,-110pt)$)  {\scriptsize    while (true) };
\node[linecode] at ($(locate-head.west)+(28pt,-120pt)$)  {\scriptsize      succ = curr.next[i].get(marked); };
 \node[linecode] at ($(locate-head.west)+(28pt,-130pt)$)  {\scriptsize     while (marked[0]) };
 \node[linecode] at ($(locate-head.west)+(28pt,-140pt)$)  {\scriptsize       s= CAS(pred.next[i],curr,succ,false,false); };
\node[linecode] at ($(locate-head.west)+(35pt,-150pt)$)  {\scriptsize        if (!s) continue retry; };
\node[linecode] at ($(locate-head.west)+(35pt,-160pt)$)  {\scriptsize        curr = pred.next[i]; };
\node[linecode] at ($(locate-head.west)+(35pt,-170pt)$)  {\scriptsize        succ = curr.next[i].get(marked); };
\node[linecode] at ($(locate-head.west)+(28pt,-180pt)$)  {\scriptsize      if (curr.key < x) };
\node[linecode] at ($(locate-head.west)+(35pt,-190pt)$)  {\scriptsize         pred = curr; curr = succ; };
\node[linecode] at ($(locate-head.west)+(28pt,-200pt)$)  {\scriptsize      else };
\node[linecode] at ($(locate-head.west)+(35pt,-210pt)$)  {\scriptsize         break; };
\node[linecode] at ($(locate-head.west)+(21pt,-220pt)$)  {\scriptsize    preds[i] = pred; };
\node[linecode] at ($(locate-head.west)+(21pt,-230pt)$)  {\scriptsize    succs[i] = curr; };
\node[linecode] at ($(locate-head.west)+(14pt,-240pt)$)  {\scriptsize   return (curr.key == key); };
...
\node[linecode,anchor=west](rmv) at ($(locate.west)+(-120pt,-280pt)$) {};
\node(rmv-head)[font={\small\tt},anchor=west] at (rmv)  {\scriptsize bool rmv(int x):};\node[linecode] at ($(rmv.east)+(0pt,-10pt)$)  {\scriptsize Node* nodeToDelete = null;};\node[linecode] at ($(rmv.east)+(0pt,-20pt)$)  {\scriptsize   int mLevel = 0;};
...
\node[linecode] at ($(rmv.east)+(21pt,-270pt)$)  {\scriptsize           else if (marked[0]) return false; };
...
\node[linecode,anchor=south west](struct) at ($(locate-head.north west)+(-4pt,8pt)$) 
{\scriptsize struct Node(int key; int topLayer; Node *nexts; bool marked);};

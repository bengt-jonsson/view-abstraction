...
to be $\tt p[1]$ if ${\tt k} = 1$, otherwise $\tt p[higher]$.
...
$\cell$ satisfies the tag $\atag$ ``at level $\klevel$, or more precisely that
\begin{itemize}
\item
...
\end{itemize}
Intuitively, $\cell \lsatfrag{\klevel}{\thread}{\sconf} \atag$ denotes that
$\cell$ satisfies $\atag$ ``at level $\klevel$'' in the sense that
array fields of $\cell$ are considered at index $\klevel$.
...
where $\inat{\frag}$ and $\outat{\frag}$ are tags
    ...
    Each skiplist fragment is either a {\em level-1 fragment} or
    a {\em higher-level fragment}.
...
\end{itemize}
Intuitively, a set of heap states is represented a set of fragments, in which
...
configurations to sets of fragments. For a local symbolic configuration
...
The symbolic postcondition computation ensures that the symbolic
representation
...
with $\sconf \satconf{\thread} \tuple{\locsymbconf,\ostate,\symbrep(\tuple{\locsymbconf,\ostate})}$,
...
\item First, the level-1 fragments of $\fragset'$ are computed, based on
  the level-1 fragments in $\symbrep(\tuple{\locsymbconf,\ostate})$.
\item
  Thereafter, the higher-level fragments of $\fragset'$ are computed, based
  on the higher-level fragments in $\symbrep(\tuple{\locsymbconf,\ostate})$ and
  how fragments in $\symbrep(\tuple{\locsymbconf,\ostate})$ are transformed
...
\paragraph{Construction of level-1 fragments}
...
modifying fragments in $\symbrep(\tuple{\locsymbconf,\ostate})$ to reflect the effect of the assignment. 
...
therefore investigate whether the set of fragments $\symbrep(\tuple{\locsymbconf,\ostate})$
...
The postcondition computation performs this investigation in a
...
\paragraph{Construction of higher-level fragments}
...
threads).
Here, we describe the intersection operation on level-1 fragments. The
intersection operation is completely analogous for higher-level fragments.
\bjcom{The following sentence must be adapted}
...
\item If $\cell$ is accessible to $\thread_1$, but not to $\thread_2$, and
  ...
  This can happen only if
  $\greachfromat{\inof{\frag_1}} =  \greachfromat{\inof{\frag_2}} = \emptyset$,
  and $\gtagat{\outof{\frag_1}} = \gtagat{\outof{\frag_2}}$,

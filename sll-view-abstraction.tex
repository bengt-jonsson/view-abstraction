...
Consider a cell $\cell$, which is reachable from thread $\thread$.
\bjcom{This notion needs to be defined}
Define
\begin{itemize}
\item
...
is a triple $\sllviewtuple$, where $\inputtag$ and $\outputtag$ are tags,
and where $\datarel$ is a subset of $\set{<, =, >}$.

Let $\conf$ be a configuration of a program containing a call $\cell$
that is accessible by thread $\thread$. For a fragment $\sllviewtuple$,
let $\cell \lhd \sllviewtuple$ denote that
$\cell \lhd \inputtag$, that
$\mathtt{next}(\cell) \lhd \outputtag$,
\bjcom{What to do with {\tt null} here etc?}
and that
...
\item each variable that assumes values in $\intgrs$ is mapped to
  ...
  of that equals the value of register $x$.
\end{itemize}
A {\em symbolic configuration} is a pair consisting of a local symbolic
configuration, and a set of fragments.

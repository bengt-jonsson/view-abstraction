...
%% In section~\ref{subsect:skiplists}, we consider programs operating
%% on skiplists, and in Section~\ref{subsect:skiplists}, we consider programs operating arrays of singly linked lists.
...
Fragment abstraction represents heap cells by a combination of local and
...
\quycom{The reason why the last case is correct is that when a pointer of $\tt x$, the $\reachfrom$ of the fragment $v$ will be changed because $v$ is reachable from $v_x$. Therefore we have to subtract reachfrom of $v_x$ from the reachfrom of $v$. However, we do not now which observers variables show we subtract. Because a observer can appear at more than 1 position in the heap. In this case we cove all possible cases.   }  
...
\todo[inline]{New Question to Quy. BUT some of the data abstraction is
  about observer registers. Mustn't this Agree?} \quycom{for data abstraction about observer register, then they must agree.}
...
\item
  ...
  \todo[inline]{Quy: Could you make a simple diagram which illustrates all the
    relations between different fragments in this case?} \quycom{Did you mean that I should make the picture of some fragment and show how they are related. I do not really understand your question here}
\end{enumerate}
...
\quycom{This case is very simple because we do not have reachfrom and reachto via higher level. Therefore, we do not need to update reachfrom and reachto information of consistent fragments as we do for 0-level. For each pair of compatible higher level fragments $\frag_x$, $\frag_y$ in $\fragset$ such that $\lvarof{x} \in \pvarsat{\inat{\frag_x}}$, $\tt y \in \pvarsat{\inat{\frag_y}}$. We just create a higher level view $\frag_{new}$ of form
    $\fragtupleat{\frag_{new}}$
    $\tagat{\inat{\frag_{new}}} = \tagat{\inat{\frag_x}}$ and $\tagat{\outat{\frag_{new}}} = \tagat{\inat{\frag_y}}$ except that $\reachfromat{\outat{\frag_{new}}} = \reachfromat{\inat{\frag_y}} \cup \reachfromat{\inat{\frag_x}}$,
    and $\datarelat{\frag_{new}} = \set{<,=,>}$.}

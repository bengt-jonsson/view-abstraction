...
\caption{Example of View Abstraction}
\label{lazylist}
\end{figure}
\end{center}
\section{View Abstraction: Singly Linked Lists}
\label{sec:annotations}
This section describes how we employ view abstraction as a symbolic
...
We first introduce the abstraction at the heart of the method. The abstraction takes its input as a set of pre-defined views, and any heap is ``broken down into pre-defined views''. A view retains the information about a particular part of a heap. This choice of views defines the level of abstraction that transforms heap into views. The abstract function $\alpha_k$ maps a heap into a set of views. Whereas, the concretization function $\gamma_k$ inputs a set of views, and returns the set of heaps that can be reconstructed from these views. 
\paragraph{Example:} Let us show an example of how a singly linked list (SLL) is split into a set of views. Fig.~\ref{lazylist}(a) shows an example of a concrete shape of a singly linked list. Each cell contains the values of $\tt val$, $\tt mark$, and $\tt lock$ from top to bottom, where \tick \; denotes $\tt true$, and \cross \; denotes $\tt false$ (or $\tt free$ for $\tt lock$) and the value of $\tt val$ is denoted by a pair of the observer register $\tt z$ and a subset of $\set{<, =, >}$. There are two threads $1$ and $2$ with two local variables $\tt c_1$ and $\tt c_2$. There are two global variables $\tt H$ and $\tt T$ pointing to the head and tail of the list. The observer register $\tt z$ has value 8. Fig.~\ref{lazylist}(b) shows the result of splitting the list in Fig.~\ref{lazylist}(a) into views where each heap view is a small list of two nodes. Fig.~\ref{lazylist}(c) shows the abstraction of views in Fig.~\ref{lazylist}(b) where for each cell $\cell$, the value of $val$ is abstracted to a subset in $\tt \{<z, =z, >z\}$, the reachability relation between $\cell$ and global variables and observer registers is abstracted to the predicate $\tt X \mapsto \tt Y$ where $\tt X$, $\tt Y$ are sets of global variables and observer registers, $\tt i$ or $\tt o$ states that $\cell$ is an input or output cell.

\paragraph{View Definition:} A heap view of size $\tt k$ is represented as \begin{center}
	${\tt v} = \viewconf$ where 
\end{center} 

%We first introduce the abstraction at the heart of the method. The abstraction takes its input as a set of pre-defined views parameterized by a constant $\tt k$, and any heap is “broken down into pieces” of size (at most) k, called views. A view retains the information about $\tt k$ connected cells from a heap and abstracts away the other cells. This choice defines the level of abstraction that transforms heap into views.There is a certain freedom in what information to retain and what to abstract away. The abstract function $\alpha_k$ maps a heap into a set of heap views of sizes $\tt k$. The concretization function $\gamma_k$ inputs a set of heap views, and returns the set of heaps that can be reconstructed from the these views. Let us represent symbolic representations of heap views. A heap view of size $\tt k$ is represented as ${\tt \Omega} = \viewconf$ where 
\begin{itemize}
...
\item  ${\tt reachto}$ maps each index $\tt i \in \indices$ to a set of
 ...
 \item $\tt \phi$ maps each pair of indices to a subset of $\set{<, =, >}$. Intuitively, $\tt \phi(i_1, i_2)$ presents the ordering between $\tt i_1.d$ and $\tt i_2.d$ for  $\intgrs$-field $\tt d$.
  ...
  \end{itemize}
For a view $\tt v$ and index $\tt i$, let $\tt tag(v,i)$ be the tuple $\tt \tuple{\tt vars(i), data(i),{\tt reachfrom}(\tt i), {\tt reachto}(\tt i), private(i )}$. Let $\tt gtag(v,i)$ be the tuple $\tt \tuple{\tt gvars(i), gdata(i),{\tt greachfrom}(\tt i), {\tt reachto}(\tt i), private(i )}$ where $\tt gvars(i)$ is the set of global variables in $\tt vars(i)$ and $\tt greachfrom(i)$ is the set of global variables in $\tt reachfrom(i)$. Note that in each view $\tt v$, for two indices $\tt i_1$, $\tt i_2$ where $\tt next(i_1) = i_2$, we always have $\tt {\tt reachfrom}(\tt i_1) \subseteq \tt {\tt reachfrom}(\tt i_2)$ and $\tt {\tt reachto}(\tt i_1) = \tt {\tt reachto}(\tt i_2) \cup X$ where $\tt X$ is the set of global variables in ${\tt vars}(\tt i_1)$ and observer registers $\tt z$ which are equal to $\tt i_1.d$ for $\intgrs$-field $\tt d$. 
%\paragraph{Abstract Configuration}:
%A abstract {\it configuration} of a program $\prog$ in the view of a single thread $\thread$
% is a tuple
...
%\item $\tt S$ is a set of views.
%\end{itemize}
%
%% We use $\confsetof\prog$ to denote the set of configurations

\section{Abstract transformers for singly linked lists}
%For each operation $\tt op$ in the intermediate representation of the analysed program corresponding to the function $\tt f_{\tt op}$ on concrete configurations $\tt F =\tuple{\athreads,\lstatemapping,\heap}$ , we define an abstract transformer ${\tt \Gamma}_{\tt op}$ on a set of views $\tt A$ such that the result of ${\tt \Gamma}_{\tt op}(A)$ denotes the set $\tt f_{\tt op}(F)$. Let us present the abstract transformers corresponding to some operations on the set $\mathbb A$. Two views $\tt \Omega_1$ and $\tt \Omega_2$ are relevant if (I) they have the same program counter and observer register and (II) their global relationship are consistent. The abstract transformer is performed by several following steps:
%\begin{enumerate}
%\item Combine all relevant views $\tt \Omega_1$,..,$\tt \Omega_n \in \mathbb A$ which contains variables appearing in the operation. Let $\tt \Omega^c_1$,..,$\tt \Omega^c_m$ be the result of the combination.
 %\item For each view $\tt \Omega^c_i$ where $0 \leq \tt i \leq m$, compute the abstract transformer $\tt \Omega_p = {\Gamma}_{\tt op}(\tt \Omega^c_i)$, then split $\tt \Omega_p$ into set of new views 
%\end{enumerate}
Let us show how to perform the abstract transformer for singly linked list programs on the set of views $\tt S$ depending on the particular statement. In this case, we consider views of size $2$ where $\tt I_{inp} = \{i\}$, $\tt I_{out} = \{o\}$,$\tt I_{int} = \emptyset$ and $\tt next(i) = o$, and views of size $1$ where $\tt I_{inp} = \{i\}$, $\tt I_{out} = \emptyset$, $\tt I_{int} = \emptyset$ and $\tt next(i) = null$ or $\tt next(i) = \bot$. Before describing in detail to compute abstract transformers, let us define several important concepts.
...
%\end{itemize}
\paragraph{View Consistency:} For two views $\tt v_1$ and $\tt v_2$ with sizes of 2, let $\tt i$ and $\tt o$ be the {\em input} and {\em output} indices of $\tt v_1$, let $\tt j_i$ and $\tt j_o$ be the {\em input} and {\em output} indices of $\tt v_2$.
 \begin{itemize}
 ...
 \item $\tt v_1$ and $\tt v_2$ is branch consistent, denoted as $\tt v_1 \leftrightarrow_S v_2$ if $\tt tag( v_1,\tt o) = tag(v_2,\tt j_o)$, $\tt G_1 \cap G_2 = \emptyset$ and $\tt v_1.{\tt reachto}(\tt i) = v_2.{\tt reachto}(\tt j_i)$ where $\tt G_1$ is the set of global variables in $\tt v_1.{\tt reachfrom}(\tt o)$,  $\tt G_2$ is the set of global variables in $\tt v_2.{\tt reachfrom}(\tt j_o)$. 
    ...
    \end{itemize}
For two views $\tt v_1$ and $\tt v_2$ with sizes of 1, let $\tt i$ be the {\em input} index of $\tt v_1$, let $\tt j_i$ be the {\em input} index of $\tt v_2$.
 \begin{itemize}
 ...
 \end{itemize}
For two views $\tt v_1$ and $\tt v_2$, let we define several more relations between them as following:
\begin{itemize}
...
\paragraph{View Intersection:} Let us describe the intersection of two views $\tt v_1 \in S_1$ and $\tt v_2 \in S_2$ denoted as $\tt v_1 \sqcap v_2$. Firstly, we consider the case where both $\tt v_1$ and $\tt v_2$ have size 2.
 \begin{itemize}
...
% 
% \end{itemize}
\paragraph{Local Abstract Transformers:} Firstly, let us show the abstract transformer on the set of view $\tt S$ in the view of the concurrent thread. Let $\tt kill(x,v)$ be the procedure to remove $\tt x$ if it exists from $\tt v.{\tt reachfrom}(\tt i)$, $\tt v.{\tt reachfrom}(\tt o)$, $\tt v.{\tt reachto}(\tt i)$, $\tt v.{\tt reachto}(\tt o)$, $\tt v.{\tt vars}(\tt i)$, and $\tt v.{\tt vars}(\tt o)$. For each program statement, let $\tt S_{post}$ be the set of views after executing the statement. Let $\tt S_{post}$ be initialized as the empty set.
\begin{itemize}

\item ${\tt x := y}$: The transformer is performed as following: For each view $\tt v_y \in S$ where $\tt y \in v_y.{\tt vars}(\tt i)$,
\begin{enumerate}
\item  for each view $\tt v$ where $\tt v \hookrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
...
\item for each view $\tt v$ where $\tt v \overset{+}\hookrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v_3$ where $\tt v_y \overset{*}\hookrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v$ where $\tt v_y \overset{*+}\leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \setminus \{x\}$ if $\tt x$ is a global variable,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v$ where $\tt v_y \overset{*o}\leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \setminus \{x\}$ if $\tt x$ is a global variable,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
%\item for each view $\tt v'$ in $\tt S_{post}$, assign $\tt v'.data.(i).(y.d)$ to $\tt v'.data.(i).(x.d)$ for $\tt i \in \{i, o\}$.
\end{enumerate}
...
\item ${\tt x := y.next}$: The local abstract transformer is quite similar to the previous case with slightly differences. For each view $\tt v_y \in S$ where $\tt y \in {\tt vars}(\tt i)$,
\begin{enumerate}
\item  for each view $\tt v$ where $\tt v \overset{*}\hookrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v$ where $\tt v_y \hookrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ then
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \cup \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \cup \{x\}$ if $\tt x$ is a global variable,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v$ where $\tt v_y \leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize} 
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \cup \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \setminus \{x\}$ if $\tt x$ is a global variable,
...
\item for each view $\tt v$ where $\tt v_y \overset{+}\hookrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \cup \{x\}$ if $\tt x$ is a global variable,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
\item for each view $\tt v$ where $\tt v \overset{+o}\leftrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that 
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item $\tt v'.i.reachfrom = v.i.reachfrom \setminus \{x\}$ if $\tt x$ is a global variable,
...
\end{itemize}
and add $\tt v'$ to $\tt S_{post}$,
%\item for each view $\tt v'$ in $\tt S_o$, assign $\tt v'.data.(i).(y.d)$ to $\tt v'.data.(i).(x.d)$ for $\tt i \in \{i, o\}$.
%
\end{enumerate}	
...
\item  ${\tt x.next:= y}$: The local abstract transformer is performed by several steps as follows: For each pair of views $\tt v_x$, $\tt v_y$ in $\tt S$ where $\tt x \in v_x.{\tt vars}(\tt i)$, $\tt y \in v_y.{\tt vars}(\tt i)$, and $\tt v_x \overset{*}\hookrightarrow v_y$ or $\tt v_y \overset{*}\hookrightarrow v_x$ or $\tt v_x \overset{**}\leftrightarrow v_y$
\begin{enumerate}
\item let $\tt v_{new}$ be the view of size 2 where $\tt tag(v_{new},i) = tag(v_x, i)$ and $\tt tag(v_{new},o) = tag(v_y,i)$ except that $\tt v_{new}.o.reachfrom = v_y.i.reachfrom \cup v_x.i.reachfrom$,
\item  for each view $\tt v \in S$ where $\tt v \overset{*}\hookrightarrow_S v_x$, we do as follows: For each subset $\tt regs$ of observer registers in $\tt v.reachto(\tt i) \cap \tt v_x.i.reachfrom$
\begin{itemize}
...
\item for each view $\tt v$ where $\tt v_y \overset{*}\hookrightarrow_S v$ or $\tt v_y = v$, 
\begin{itemize}
	\item create  $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
...
\item add $\tt v'$ to $\tt S_{post}$
\end{itemize}
\item for each view $\tt v$ where $\tt v_x \overset{**}\leftrightarrow_S v$ and either $\tt v \hookrightarrow_S v_y$ or $\tt v_y \overset{*o}\leftrightarrow_S v$, 
\begin{itemize}
...
\item for each view $\tt v$ where $\tt v_x \overset{*}\hookrightarrow_S v$ and either $\tt v \hookrightarrow_S v_y$ or $\tt v_y \overset{*o}\leftrightarrow_S v$,  
\begin{itemize}
...
\item for each view $\tt v$ where $\tt v_x \overset{**}\leftrightarrow_S v$ and either $\tt v \overset{+}\hookrightarrow_S v_y$ or $\tt v \overset{*+}\leftrightarrow_S v_y$, 
\begin{itemize}
...
\item for each view $\tt v$ where $\tt v_x \overset{*}\hookrightarrow_S v$ and either $\tt v \overset{+}\hookrightarrow_S v_y$ or $\tt v_y \overset{*+}\leftrightarrow v$, then for each subset $\tt regs$ of observer registers in $\tt v.i.reachfrom \cap \tt v_x.i.reachfrom$, 
\begin{itemize}
\item create $\tt v'$ which is same as $\tt v$, except that $\tt v'.i.reachfrom = (\tt v.i.reachfrom \setminus v_x.i.reachfrom) \cup regs$. 
...
Let us show how to perform the abstract transformer for skip-list programs on the set of views $\tt S$ depending on the particular statement.  We consider views of size $2$ where $\tt I_{inp} = \{i\}$, $\tt I_{out} = \{{o}\}$, and $\tt next(i) = {o}$ and views of size $1$ where $\tt I_{inp} = \{i\}$, $\tt I_{out} = \emptyset$, and $\tt next(i) = null$ or $\tt next(i) = \bot$. For each view $\tt v$, let $\tt v.level \in \{1,2\}$ be the level of $\tt v$.

\paragraph{Local Abstract Transformers:} First, let us show the abstract transformer on the set of view $\tt S$ in the view of the concurrent thread. Let $\tt S_1$ be set of views of level 1 in $\tt S$, $\tt S_2$ be set of views of level 2 in $\tt S$. For each program statement, let $\tt S_{post}$ be the set of views after executing the statement. Let $\tt S_{post}$ be initialized as the empty set.  Let $\tt R$ be the set of pairs of views. Intuitively, in each element in $\tt R$, the second view is the transformation of the first view. Let $\tt R$ be initialized as the empty set.
\begin{itemize}
\item ${\tt x := y}$: The transformer is performed as follows:  For each view $\tt v_y \in S_1$ where $\tt y \in v_y.{\tt vars}(\tt i)$, 
%\begin{itemize}
\begin{enumerate}
\item  for each view $\tt v \in S_1$ where $\tt v \hookrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \cup \{x\}$,
...
\item for each view $\tt v \in S_1$ where $\tt v \overset{+}\hookrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt R$
\item for each view $\tt v \in S_1$ where $\tt v_y \overset{*}\hookrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt S'$
\item for each view $\tt v \in S_1$ where $\tt v_y \overset{*o}\leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt S'$
%\item for each view $\tt v'$ in $\tt S_{post}$, assign $\tt v'.data.(i).(y.d)$ to $\tt v'.data.(i).(x.d)$ for $\tt i \in \{i, o\}$.
\item for each view $\tt v \in S_2$ we do as follows. For each $\tt (v_1,v_1')$, $\tt (v_2,v_2') \in R$, for each pair of indices $\tt i_1$, $\tt i_2$ such that $\tt i_1 \in \{v_1.i, v_1.o\}$, $\tt i_2 \in \{v_2.i, v_2.o\}$, $\tt tag(v,i) = tag(v_1, i_1)$, and $\tt tag(v,o) = tag(v_2, i_2)$. Create $\tt v'$ which is same as $\tt v$ except that 
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% X := Y.NEXT1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item ${\tt x := y.next1}$: The local abstract transformer is quite similar to the previous case with slightly differences. For each view $\tt v_y \in S$ where $\tt y \in {\tt vars}(\tt i)$,
%\begin{enumerate}
%
%%1%
%\item  for each view $\tt v$ where 
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt R$
\item for each view $\tt v \in S_1$ where $\tt v_y \hookrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ then
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \cup \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt R$
\item for each view $\tt v \in S_1$ where $\tt v_y \leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that
\begin{itemize} 
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \cup \{x\}$,
\item if $\tt x$ is a global variable
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt R$
\item for each view $\tt v \in S_1$ where $\tt v_y \overset{*+}\leftrightarrow_S v$, create $\tt v'$ which is same as $\tt v$ except that 
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\{$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
\end{itemize}
then add $\tt v'$ to $\tt S_{post}$, and $\tt (v,v')$ to $\tt R$
\item for each view $\tt v \in S_1$ where $\tt v \overset{+o}\leftrightarrow_S v_y$, create $\tt v'$ which is same as $\tt v$ except that 
\begin{itemize}
\item $\tt v'.i.vars = v.i.vars \setminus \{x\}$,
\item $\tt v'.o.vars = v.o.vars \setminus \{x\}$,
\item if $\tt x$ is a global variable
...
\item  ${\tt x.next1:= y}$: The local abstract transformer is performed by several steps as follows: For each pair of views $\tt v_x$, $\tt v_y$ in $\tt S_1$ where $\tt x \in v_x.{\tt vars}(\tt i)$, $\tt y \in v_y.{\tt vars}(\tt i)$, and $\tt v_x \overset{*}\hookrightarrow v_y$ or $\tt v_y \overset{*}\hookrightarrow v_x$ or $\tt v_x \overset{**}\leftrightarrow v_y$, let $\tt R_1$, $\tt R_2$, $\tt R_3$, $\tt R_4$, $\tt R_5$, $\tt R_6$ be initialized as $\tt R$,  
\begin{enumerate}
\item let $\tt v_{new}$ be the view of size 2 and of level 1 where $\tt tag(v_{new},i) = tag(v_x, i)$ and $\tt tag(v_{new},o) = tag(v_y,i)$ except that $\tt v_{new}.o.reachfrom = v_y.i.reachfrom \cup v_x.i.reachfrom$,
\item  for each view $\tt v \in S_1$ where $\tt v \overset{*}\hookrightarrow_S v_x$, we do as follows: For each subset $\tt regs$ of observer registers in $\tt v.reachto(\tt i) \cap \tt v_x.i.reachfrom$
...
\item for each view $\tt v \in S_1$ where $\tt v_y \overset{*}\hookrightarrow_S v$ or $\tt v_y = v$, 
\begin{itemize}
	\item create  $\tt v'$ which is same as $\tt v$ except that
\begin{itemize}
 \item $\tt \tt v'.i.reachfrom = v_x.i.reachfrom \cup \tt v.i.reachfrom$, 
...
\end{itemize}
\item for each view $\tt v \in S_1$ where $\tt v_x \overset{**}\leftrightarrow_S v$ and either $\tt v \hookrightarrow_S v_y$ or $\tt v_y \overset{*o}\leftrightarrow_S v$, 
\begin{itemize}
\item  create $\tt v'$ which is same as $\tt v$ except that $\tt \tt v'.o.reachfrom = v_x.i.reachfrom \cup \tt v.o.reachfrom$, 
 \item add $\tt v'$ to $\tt S_{post}$, 
...
\item for each view $\tt v \in S_1$ where $\tt v_x \overset{*}\hookrightarrow_S v$ and either $\tt v \hookrightarrow_S v_y$ or $\tt v_y \overset{*o}\leftrightarrow_S v$,  
\begin{itemize}
\item create $\tt v'$ which is same as $\tt v$ then except that $\tt \tt v'.o.reachfrom = v_x.i.reachfrom \cup \tt v.o.reachfrom$, 
\item for each subset $\tt regs$ of observer registers in $\tt v'.i.reachfrom \cap \tt v_x.i.reachfrom$
\begin{itemize}
...
\item for each view $\tt v \in S_1$ where $\tt v_x \overset{**}\leftrightarrow_S v$ and either $\tt v \overset{+}\hookrightarrow_S v_y$ or $\tt v \overset{*+}\leftrightarrow_S v_y$, 
\begin{itemize}
\item create $\tt v'$ which is same as $\tt v$
\item  add $\tt v'$ to $\tt S_{post}$,
...
\item for each view $\tt v \in S_1$ where $\tt v_x \overset{*}\hookrightarrow_S v$ and either $\tt v \overset{+}\hookrightarrow_S v_y$ or $\tt v_y \overset{*+}\leftrightarrow v$, then for each subset $\tt regs$ of observer registers in $\tt v.i.reachfrom \cap \tt v_x.i.reachfrom$, 
\begin{itemize}
\item create $\tt v'$ which is same as $\tt v$, except that $\tt v'.i.reachfrom = (\tt v.i.reachfrom \setminus v_x.i.reachfrom) \cup regs$. 
 \item for each set $\tt regs'$ of observer registers in $\tt v'.o.reachfrom \cap \tt v_x.i.reachfrom$,  
\begin{itemize}

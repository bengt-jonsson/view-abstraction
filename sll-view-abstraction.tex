...
\item the cells of the heap that are accessible to thread $\thread$, and
\item a predicate $\private$ over heap cells accessible to $\thread$, which
    is true for cells that are not accessible to any other thread.
\end{inparaenum}

\paragraph{Shape Abstraction.}
We represent the part of the heap that is accessible to $\thread$ by a
set of {\em fragments}. 
A fragment consists of a pair of heap cells that are connected by a pointer,
in which each of the two heap cell is labeled by
\begin{inparaenum}[(i)]
\item the pointer variables (either global or local to $\thread$) that point
  to it, and
\item globally significant cells that can reach it and be reached from it
  (by following a chain of {\tt next}-pointers); these cells are represented by
  the corresponding global pointer variables and observer registers.
\end{inparaenum}
A set of fragments represents the set of heap views
in which each pair of pointer-connected nodes is represented by some
fragment in the set.
Inuitively, a set of fragments describes the set of heap views
that can be formed by ``piecing together'' fragments in the set.
This ``piecing together'' must
be both locally consistent (connecting only fragments that agree on their
common node), and globally consistent (respecting the reachability
information wrp.\ to globally significant cells).

\paragraph{Data Abstraction.}
We apply a natural data abstraction to the local state of a thread, the
...
data values in $\ddomain$ by their relative ordering. The data abstraction
can be conveniently represented by defining an abstract domain for
each concrete domain of data values.
...
\item
  ...
  where $\mathit{me}$ means that the lock is held by $\thread$,
  where $\mathit{other}$ means that the lock is held by some other thread, and
  $\mathit{free}$ means that the lock is free.
\item For the concrete domain $\ddomain$ of data values, the abstract domain
  is the set of mappings from local variables ranging over $\ddomain$ and from
  observer registers to the set $\set{<,=,>}$.
  An element in the abstract domain
  represents a concrete data value $\tt d$
  if it maps each local variable and observer register with value $\tt d'$
...
Let us next define the combination of our data abstraction and fragment
abstraction.
Define a {\em tag} as a tuple $\atag = \tagtuple$, where
\begin{itemize}
\item
...
in a configuration $\conf$, and a tag
...
\item
  ...
  \end{inparaenum}
\item
  $\private$ is $\true$ only if $\cell$ is not accessible to any other thread
  than $\thread$ \bjcom{This needs fixing}.
  %% $\private$ is $\true$ if
...
that the ${\tt next}$ field of $\cell$ points to a cell $\cell'$ such that

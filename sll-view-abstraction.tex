...
\item observer registers whose value is equal to the value of the $\intgrs$-field of some cell from which $\cell$ is reachable.
  \bjcom{Question to Quy: must the ``cell from which $\cell$ is reachable''
    be accessible from $\thread$?} \quycom{yes, it must be accessible from $\thread$}
  \end{enumerate}
...
A {\em local symbolic configuration} consists of a mapping from local variables
...
\item each variable that assumes values in $\intgrs$ is mapped to
  ...
  each observer register $x  \in \ovarset$, whether or not the value
  of that equals the value of register $x$.
\item \bjcom{We may need constraints for data and timer variables?}
\end{itemize}
...
\bjcom{Go over the last paragraph again}
...
\bjcom{This past text can be improved}
In general, updating
...
\item  $\frag_1 \hookrightarrow_S \frag_2$ denote that $\tagat{\outat{\frag_1}} = \tagat{\inat{\frag_2}}$;
  \todo[inline]{Quy: Is it not enough that
    $\tagat{\outat{\frag_1}}$ and $\tagat{\inat{\frag_2}}$ are consistent?} \quycom{Yes, its enough}
...
\item  $\frag_1 \leftrightarrow_S \frag_2$ denote that $\tagat{\outat{\frag_1}} = \tagat{\outat{\frag_2}}$ and that 
  \todo[inline]{Quy: Is it not enough that
    $\tagat{\outat{\frag_1}}$ and $\tagat{\outat{\frag_2}}$ are consistent?}
  the global variables in  $\reachfromat{\inat{\frag_1}}$ are disjoint from
  those in $\reachfromat{\inat{\frag_2}}$.
  $\reachfromat{\inat{\frag_1}} \cap \reachfromat{\inat{\frag_2}} = \emptyset$ \quycom{$\reachfromat{\inat{\frag_1}} \cap \reachfromat{\inat{\frag_2}} = \emptyset$ here is not correct, we only need the global variables of reachfrom and reachto in this case}
  \todo[inline]{Quy: Could you check this, and compare with your definition:
    I could not see that your definition was the right one. It may depend on
    whether $\frag_1$ and $\frag_2$ are from the same or different threads}  \quycom{In my definition, the consistent here is defined for two views in the same threads. With the view from other thread, I use the notion of intersection.}
  intuitively, this means that if
  $\cell_1 \lhd \frag_1$ and  $\cell_2 \lhd \frag_2$ then it is possible that
...
\end{itemize}
and add $\frag'$ to $\tt S_{post}$,
%\item for each fragment $\frag'$ in $\tt S_{post}$, assign $\frag'.data.(i).(y.d)$ to $\frag'.data.(i).(x.d)$ for $\tt i \in \{i, o\}$.
\end{enumerate}
\end{description}

\todo[inline]{After here is old text}

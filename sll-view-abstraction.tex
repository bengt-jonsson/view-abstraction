...
configuration $\locsymbconf$, defined in the natural way. Let an augmented
...
%%   such a way that makes it necessary to extend $\symbrep(\locsymbconf)$.
  ...
  $\fragset' \in \symbrep(\tuple{\locsymbconf,\ostate'})$.
...
therefore investigate whether the set $\symbrep(\auglocsymbconf)$
...
fragments which are satisfied by the configuration after the statement, implying
that any cell $\cell$ which is accessible to $\thread$ after
the statement satisfies some fragment in $\fragsetpost$.
The cell $\cell$ must satisfy some
fragment $\frag$ in $\fragset$ before the statement,
and must be in the same heap state as the cell pointed to by $\gvarof{y}$. 
